+++
title = "Privacy Policy"
description = "Privacy policy for the RoastPOS marketing website."
eyebrow = "Legal"
draft = false
+++

This privacy policy explains how the RoastPOS marketing site handles information. RoastPOS is an open-source, self-hosted project, and this site is intended primarily to provide project information, repository links, and contact details.

## Website data

This site may collect limited technical data that is normally available to web infrastructure, such as IP address, browser details, request timing, and standard server logs. This data is used for basic site operation, security, and debugging. It is not sold to third parties.

## Contact and correspondence

If you contact us by email at [hello@naughtybeanconsulting.com](mailto:hello@naughtybeanconsulting.com), we may retain your message and contact details for the purpose of responding to your inquiry, maintaining project communications, or handling open-source collaboration discussions.

## Donations and payment providers

If you choose to use a donation link from this site, payment processing will be handled by a third-party payment provider. We do not control that provider's data handling practices. You should review the provider's privacy policy before submitting payment information.

## Open-source project data

If you interact with the RoastPOS GitHub repository, those interactions are governed by GitHub's own privacy terms and platform policies.

## Self-hosted deployments

The RoastPOS software is self-hosted by its users. Data processed inside a self-hosted RoastPOS deployment — including customer orders, sales records, and staff details — is controlled entirely by the operator running that deployment, not by this marketing site or Naughty Bean Consulting.

Operators of self-hosted RoastPOS installations are responsible for their own privacy compliance, including applicable data protection regulations in their jurisdiction.

## Analytics

This site does not currently use third-party analytics tracking. If analytics are added in future, this policy will be updated and any tracking will be disclosed.

## Cookies

This site does not set any tracking cookies. Basic session cookies may be set by the hosting infrastructure for operational purposes.

## Your rights

Where applicable under law, you may have the right to access, correct, or request deletion of any personal data we hold in connection with this site. Contact us at [hello@naughtybeanconsulting.com](mailto:hello@naughtybeanconsulting.com) with any such requests.

## Contact

Questions about privacy can be sent to [hello@naughtybeanconsulting.com](mailto:hello@naughtybeanconsulting.com).
